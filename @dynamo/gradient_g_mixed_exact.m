...
%
% Uses H{t}, P{t_c}, U{t_c}, U{t_tau+1} and L{t+1}.
...
if c < 0
    ...
    ret = self.seq.tau(t) * self.seq.fields_deriv(t, c) * ...
          trace_matmul(self.cache.L{t+1, k}, dPdu * self.cache.U{t, k} * self.cache.P{t, k}');
end

% this is a single-use function (only used with error_real) so
% no VUdagger is needed, we can do the -1 here directly
ret = -2 *ret;
